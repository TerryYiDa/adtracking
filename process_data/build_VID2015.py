...
import collections
bounding_box = collections.namedtuple('box', ['x', 'y', 'width', 'height'])
  ...
  def dataset_iterator(self, video_dirs):
    ...
    for iter_ in range(iter_num):
      ...
      for index in range(num_videos):
        ...
        with open(osp.join(video_dir, 'ground_truth.txt'), 'r') as f:
            tmp = []
            for line in f.readlines:
                col = line.strip().split(',')
                tmp.append(bounding_box(int(col[0]), int(box[1]), int(box[2]), int(box[3]))
            instance_box.append(tmp)

      data_iter.num_videos = len(instance_videos)
      data_iter.instance_videos = instance_videos
      data_iter.instance_box = instance_box
      yield data_iter
...
def main():
  ...
  for i, data_iter in enumerate(dataset.dataset_iterator(train_dirs)):
    ...
    train_imdb['box'] += data_iter.instance_box
